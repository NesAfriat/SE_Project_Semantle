...
def get_distance_of_word(kv_model, w1, w2, func):
    return func(kv_model[w1], kv_model[w2])
    ...
    with open(distance_path, 'x') as distance_file, open(vector_path, 'x') as vector_file:
        for word1 in vocabulary:
            ...
            for word2 in vocabulary:
                key = f"{word1}{word2}"
                distance = get_distance_of_word(kv_model, word1, word2, func)
                value = f"{distance:.6f}"
...
if __name__ == "__main__":
    for dist_function_name, dist_function in dist_dict.items():
        dist_func_name = dist_function_name
        ...
        for i, model_name in dict_model.items():
            if i == 3:
                ...
                distances, vectors = save_word_combinations(my_model, model_vocab, model_name, dist_path, vec_path, dist_func)
