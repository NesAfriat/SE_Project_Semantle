...
class GameBuilder():
    ...
    def build_agent_host(self,game):
        ...
        self.host_builder.with_model(game["host_model"], self.model_factory)
        self.host_builder.with_distance_function(game["distance_function"])
        host = self.host_builder.get_host()
        self.agent_builder.create_agent_and_model(game["agent"],host,game["model"], self.model_factory)
